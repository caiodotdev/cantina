...
class ListFull(LoginRequiredMixin, ItemPedidoMixin, ListView):
    ...
    def search_general(self, qs):
        if 'search' in self.request.GET:
            ...
            if self.search:
                search = self.search
                qs = qs.filter(Q(id__icontains=search)| Q(pedido__cliente__icontains=search)| Q(item__id__icontains=search)| Q(qtd__icontains=search)| Q(total__icontains=search))
        return qs
...
class ItemPedidoListJson(BaseDatatableView):
    model = ItemPedido
    columns = ("id", "pedido", "item", "qtd", "total")
    order_columns = ["id", "pedido__cliente", "item__id", "qtd", "total"]
